...
function globalmassblock() {
	if (mw.config.get("wgPageName") == "Special:Globalmassblock") {
		...
		document.getElementById("mw-globalblock-anon-only").checked = getParameter("ao") == "1";
		document.getElementById("mw-globalblock-expiry-other").value = getParameter("expiry").replace("+"," ","g");
		document.getElementById("mw-globalblock-reason").value = getParameter("reason").replace("+"," ","g");
...
function globalmassblock2() {
	...
	mbcode = "globalmassblock3(wgGlobalBlocksToDo[wgGlobalBlocksToDoIndex++]);if (wgGlobalBlocksToDoIndex >= wgGlobalBlocksToDo.length) globalmassblock4();";

	wgGlobalBlocksToDoIndex = wgGlobalBlockFrame = 0;
	eval(mbcode);
	wgGlobalBlocksToDoInterval = setInterval(mbcode, (60 / parseFloat(document.getElementById("epm").value)) * 1000);
}
...
function globalmassblock3(ipToGlobalBlock) {
	...
	frames["globalblockframe" + wgGlobalBlockFrame++].location.href = wgServer + wgScript + "?title=Special:GlobalBlock&wpAddress=" +
				ipToGlobalBlock + "&__GLOBALMASSBLOCK__=1&wpExpiry=other&wpExpiryOther" + document.getElementById("expiry").value + 
				"&wpReason=" + document.getElementById("reason").value + "&wpAnonOnly=" + (document.getElementById("ao").checked ? "1" : "0");
